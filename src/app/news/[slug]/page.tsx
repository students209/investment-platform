...
export default async function NewsDetail({
  ...
  const report = await getReport('daily', slug)
      ...
      <article className="bg-white rounded-lg shadow p-8">
        <header className="mb-8 pb-6 border-b">
          <div className="flex items-center gap-3 mb-4">
            <span className="px-3 py-1 bg-emerald-100 text-emerald-700 text-sm rounded">
              每日资讯
            </span>
        ...
        <div className="prose prose-lg max-w-none">
          {report.body.split('\n').map((line, i) => {
            ...
            if (line.startsWith('> ')) {
              return <blockquote key={i} className="border-l-4 border-gray-300 pl-4 italic text-gray-600 my-4">{line.slice(2)}</blockquote>
            }
            if (line.trim() === '') {
              return <br key={i} />
            }
